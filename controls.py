    ...
    if window_exist:
        ...
        for proc in psutil.process_iter(['name']): # Verifica todos os processos do PC
            if proc.info['name'] == 'sige.exe': # Caso tenha o processo
                ...
                while menu:
                    try:
                        sige = pg.locateOnScreen('files\\assets\\sige_password.png',confidence=0.8,)
                        pg.move(sige)
                        pg.click(sige)
                        break
                    except:
                ...
                time.sleep(500/1000)
                estoque = pg.locateOnScreen('files\\assets\\estoque.png',confidence=0.8,)
                pg.move(estoque)
                pg.click(estoque)
                pg.move(500,500)
                return True
...
def verify_my_time(user: str):
    ...
    time.sleep(500/1000)
    operador = pg.locateOnScreen('files\\assets\\operador.png',confidence=0.3,region=(500,500,1800,900))
    pg.move(operador)
    pg.click(operador)
    time.sleep(500/1000)
...
def open_workman(asstec_number: str):
    ...
    while menu:
        try:
            sige = pg.locateOnScreen('files\\assets\\menu_sige.png',confidence=0.8,)
            pg.move(sige)
            pg.click(sige)
            break
        except:
